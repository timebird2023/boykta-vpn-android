...
class ServerAdapter(private val listener: Listener) :
    ...
    inner class VH(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(server: Server) {
            ...
            // Name only — never show config/VLESS URI
            ...
            // Status dot
            val dotColor = when {
                isConnected -> 0xFF00FF88.toInt()
                server.isExpired() -> 0xFFFF4444.toInt()
            ...
            if (isConnected) {
                ...
                btnConnect.setBackgroundColor(0xFF00D4FF.toInt())
                btnConnect.setTextColor(0xFF000000.toInt())
                btnConnect.isEnabled = !server.isExpired()
                btnConnect.alpha = if (server.isExpired()) 0.4f else 1f
